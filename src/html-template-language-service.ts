...
): { span: ts.TextSpan; newText: string } {
  return {
...
function translateCompetionEntry(
  ...
  const kind = vsItem.kind
    ? translateionCompletionItemKind(typescript, vsItem.kind)
...
function translateCompletionItemsToCompletionInfo(
  ...
  return {
    ...
    entries: items.items.map(x =>
      translateCompetionEntry(typescript, context, x)
    )
  };
}
...
function translateCompletionItemsToCompletionEntryDetails(
  ...
  return {
    ...
    kind: item.kind
      ? translateionCompletionItemKind(typescript, item.kind)
      : typescript.ScriptElementKind.unknown,
...
export default class HtmlTemplateLanguageService
  ...
  public getQuickInfoAtPosition(
    ...
    const documentRegions = getDocumentRegions(
    ...
    switch (languageId) {
      case "html": {
        ...
        const hover = this.htmlLanguageService.doHover(
          document,
          position,
          htmlDoc
        );
        return hover
          ? this.translateHover(hover, position, context)
          : undefined;
      }

      case "css":
        return this.styledLanguageService.getQuickInfoAtPosition(
          context,
          position
        );
    }
  ...
  public getSignatureHelpItemsAtPosition(): ts.SignatureHelpItems | undefined {
  ...
  public getReferencesAtPosition(
    ...
    const htmlDoc = this.htmlLanguageService.parseHTMLDocument(document);
    const highlights = this.htmlLanguageService.findDocumentHighlights(
      document,
      position,
      htmlDoc
    );
  ...
  public getJsxClosingTagAtPosition(
    ...
    const tagComplete = this.htmlLanguageService.doTagComplete(
      document,
      position,
      htmlDocument
    );
    ...
    switch (languageId) {
      case "html": {
        ...
        const htmlCompletions: HtmlCachedCompletionList = {
          type: "html",
          value:
            this.htmlLanguageService.doComplete(document, position, htmlDoc) ||
            emptyCompletionList
        };
      ...
      case "css": {
        const styledCompletions: StyledCachedCompletionList = {
          type: "styled",
          value: this.styledLanguageService.getCompletionsAtPosition(
            context,
            position
          )
        };
        this._completionsCache.updateCached(
          context,
          position,
          styledCompletions
        );
